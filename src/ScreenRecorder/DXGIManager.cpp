...
HRESULT DXGIOutputDuplication::AcquireNextFrame(IDXGISurface1** pDXGISurface, DXGIPointerInfo*& pDXGIPointer)
{
	...
	if(FAILED(hr))
	{
		LOG(DETAIL) << "m_DXGIOutputDuplication->AcquireNextFrame failed with hr=" << hr;
		return hr;
	}
	...
	*pDXGISurface = spDXGISurface.Detach();
	...
	if(fi.PointerPosition.Visible)
	{
		...
		if(hr == S_OK)
		{
			LOG(DETAIL) << "PointerPosition Visible=" << fi.PointerPosition.Visible
...
HRESULT DXGIManager::Init()
{
	...
	if( FAILED(hr) )
	{
		LOG(ERROR) << "Failed to CreateDXGIFactory1 hr=" << hr;
		return hr;
	}
		...
		AdapterIter++)
	{
		...
		{ 
            ...
            char buf[1024] = { 0 };
			sprintf(buf, "Display output found. DeviceName=%ls  AttachedToDesktop=%d Rotation=%d DesktopCoordinates={(%d,%d),(%d,%d)}\r\n", 
		...
		if( FAILED(hr) )
		{
			LOG(ERROR) << "Failed to create D3D11CreateDevice hr=" << hr;
			return hr;
		}
      ...
      if (!spDXGIOutput1)
      {
        LOG(WARNING) << "spDXGIOutput1 is NULL";
        continue;
      }
      ...
      if (!spDXGIDevice)
      {
          LOG(WARNING) << "spDXGIDevice is NULL";
        continue;
      }
      ...
      if (FAILED(hr))
      {
        LOG(ERROR) << "Failed to duplicate output hr=" << hr;
        continue;
      }
	...
	if( FAILED(hr) )
	{
		LOG(ERROR) << "Failed to create WICImagingFactory hr" << hr;
		return hr;
	}
...
HRESULT DXGIManager::GetOutputRect(RECT& rc)
{
	...
	for(std::vector<DXGIOutputDuplication>::iterator iter = vOutputs.begin();
		iter != vOutputs.end();
		iter++)
	{
...
void DXGIManager::DrawMousePointer(BYTE* pDesktopBits, RECT rcDesktop, RECT rcDest)
{
	...
	switch(m_pDXGIPointer->GetShapeInfo().Type)
	{
		case DXGI_OUTDUPL_POINTER_SHAPE_TYPE_COLOR:
			{
				std::unique_ptr<Bitmap> bmpBitmap(new Bitmap(dwDestWidth, dwDestHeight, dwDestWidth*4, PixelFormat32bppARGB, pDesktopBits));
				std::unique_ptr<Graphics> graphics(Graphics::FromImage(bmpBitmap.get()));
				std::unique_ptr<Bitmap> bmpPointer(new Bitmap(m_pDXGIPointer->GetShapeInfo().Width, m_pDXGIPointer->GetShapeInfo().Height, m_pDXGIPointer->GetShapeInfo().Width*4, PixelFormat32bppARGB, m_pDXGIPointer->GetBuffer()));
...
std::vector<DXGIOutputDuplication> DXGIManager::GetOutputDuplication()
{
	std::vector<DXGIOutputDuplication> outputs;
	switch(m_CaptureSource)
	{
		case CSMonitor1:
		{
			// Return the one with IsPrimary
			for(std::vector<DXGIOutputDuplication>::iterator iter = m_vOutputs.begin();
				iter != m_vOutputs.end();
				iter++)
			{
				DXGIOutputDuplication& out = *iter;
